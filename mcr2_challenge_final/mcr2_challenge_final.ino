...
std_msgs__msg__Float32 msg_sub;  //Defines a message of type float32.
...
// Control
int pwm = 0;
float controlSignal = 0.0; // 0..100%

// PID Incremental
float Kp = 1.6;
float Ki = 0.6;
float Kd = 0.02;
float e0 = 0.0, e1 = 0.0, e2 = 0.0;
  
const float alphaRPM = 0.20; 

// Tiempo de muestreo
const float Ts = 0.05; // 100 ms
...
void control_function(){
  ...
  rpm_raw = clampf(rpm_raw,0.0,RPM_MAX);
  ...
  //SATURACION
  controlSignal = constrain(controlSignal,0,1);
  //CONVERTIR A PWM
  uint32_t duty = (uint32_t)(controlSignal * PWM_MAX);
  ...
  msg_pub.data.data[2] = controlSignal;
  msg_pub.data.data[3] = rpm * dir;

  msg_vel.data = rpm * dir;
  rcl_publish(&publisher_vel, &msg_vel, NULL);
...
void setup() {
  ...
  msg_pub.data.data = (float*) malloc(4*sizeof(float));
  msg_pub.data.size = 4;
  msg_pub.data.capacity = 4;
}
...
void loop() {
  switch (state) {
    ...
    case AGENT_CONNECTED:
      EXECUTE_EVERY_N_MS(50,control_function());
...
void destroy_entities()
{
  ...
  rcl_publisher_fini(&publisher_vel,&node);
